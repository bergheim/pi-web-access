...
- Added a self-hosted Crawl4AI extraction fallback for `fetch_content`, configured with `crawl4aiBaseUrl` / `CRAWL4AI_BASE_URL` and `crawl4aiApiToken` / `CRAWL4AI_API_TOKEN`. It calls `POST /md` with the `fit` filter, runs after Firecrawl and before hosted providers, and `crawl4ai` is accepted in `fetchRouting.providers`.
